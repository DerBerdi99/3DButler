...
class OrderManager:
    ...
    def save_payment_info(self, user_id: str, method: str, last_digits: str, expiry: str):
        # First, set all existing payment methods to non-default
        self._execute_query(
            "UPDATE Payments SET IsDefaultMethod = 0 WHERE UserID = ?",
            (user_id,)
        )
    ...
    def get_order_info_by_project(self,project_id):
        """Ruft die OrderID einer Bestellung ab, die mit einem bestimmten Projekt verknüpft ist."""
        query = "SELECT OrderID, OrderStatus FROM Orders WHERE SourceProjectID = ? LIMIT 1"
        result = self._execute_query(query, (project_id,), fetch=True, fetch_one=True)
        if result:
            # Rückgabe beider Werte als Tupel
            return result['OrderID'], result['OrderStatus']
    
        # Rückgabe von zwei None-Werten, damit das Unpacking im Endpoint nicht crasht
        return None, None